...
def inference_set(directory, expected):
    ...
    return {
        ...
        "valid": count == expected and summary is not None and summary.get("image_count") == expected,
    }
...
def main():
    ...
    checks["training_logs"] = {
        "jittor": training_log(project / "logs" / f"jittor_msrs_roadscene_60e_{tag}" / "train.log"),
        "pytorch": training_log(project / "logs" / f"pytorch_msrs_roadscene_60e_{tag}" / "train.log"),
    }
    checks["checkpoints"] = {
        "jittor": single_checkpoint(project / "checkpoints" / f"jittor_msrs_roadscene_60e_{tag}", ".pkl"),
        "pytorch": single_checkpoint(project / "checkpoints" / f"pytorch_msrs_roadscene_60e_{tag}", ".pth"),
    }
